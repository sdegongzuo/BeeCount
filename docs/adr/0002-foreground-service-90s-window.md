...
最多允许 2 个 job 并发执行，多余排队。`lease_until` 字段防止多个 worker 同时领取同一个 job。用户快速连续分享多张图片时，串行处理确保每张图都有完整的执行窗口。

通知设计：
- 正在识别账单...（OCR/规则阶段）
- 正在补全账单信息...（AI 阶段）
- 记账已创建，附件稍后保存（AI 完成，附件未完成）
- 记账完成（全部完成）
- 记账已创建，剩余信息稍后补全（超时）
- 部分信息待补全，打开 App 后继续（失败）

用户点击通知：打开 App 并自动触发 `resumePendingBillingJobs()` 恢复未完成任务。
